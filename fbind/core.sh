  ...
  if ! is_mounted "$2"; then
    ...
    if grep -iq '/storage/emulated sdcardfs' /proc/mounts && echo "$@" | grep -q $prefix; then
      if echo "$extsd" | grep -q $prefix; then
        ...
        if ! grep -iq noWriteRemount $config && ! [ -f $modData/.noWriteRemount ]; then # some systems lock and reboot if this is remounted
      ...
      if [ -f $modData/.noWriteRemount ]; then
...
part() {
  ...
  if ! is_mounted "$2"; then
    ...
    if echo "$1" | grep -q '\-\-L'; then
      ...
      [ -n "$3" ] && $3 $pPath
      mount -t $(blkid | grep $pPath | sed -e 's|.*TYPE="||' -e 's|".*||') $(echo -n $extraOpts) $pPath "$2"
    fi
...
apply_config() {
  [ -f $config ] || cp -af /data/media/0/.fbind_config_backup.txt $config 2>/dev/null || touch $config
  ...
  set +x
  until [ -d /data/media/0/?ndroid ]; do sleep 2; done
